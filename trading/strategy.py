...
def _find_fvg(m15: list[Candle], trend: str) -> tuple[Optional[tuple[float, float]], str]:
    ...
    A 3-candle pattern where candle[i-1] and candle[i+1] don't overlap,
    leaving an imbalance gap. Searches last 30 candles.
    Gap must not have been fully filled since formation.
    Impulse multiplier relaxed to 1.0x (gap itself IS the impulse signal).
    """
    if len(m15) < 5:
    ...
    for i in range(len(m15) - 2, len(m15) - 2 - search_depth, -1):
        ...
        if trend == "bullish" and curr.is_bullish:
            # Bullish FVG: gap between prev.high and nxt.low
            if nxt.low > prev.high:
                fvg_low  = prev.high
                fvg_high = nxt.low
                # Check gap not fully filled
            ...
            # Bearish FVG: gap between nxt.high and prev.low
...
def _find_poi(
    ...
    """
    Gate 11: Point of Interest — OB first, FVG second, swing extreme last fallback.
    The swing extreme fallback ensures we never block a clean signal just because
    the OB/FVG hasn't printed yet — e.g. after a drought of 3-4 days on a pair.
    """
    ...
    # Swing extreme fallback — uses the sweep reference as the POI boundary
    if sweep_ref > 0:
        if trend == "bullish":
            # POI zone is just above the sweep low
            zone_low  = sweep_ref
            zone_high = sweep_ref * 1.002  # 0.2% band
...
def _entry_confirmation(m15: list[Candle], trend: str) -> tuple[bool, str]:
    """
    Gate 12: Flexible confirmation — any ONE of:
      (a) Strong candle: body_ratio >= 0.55 in trend direction
      (b) Engulfing: last candle body engulfs prior candle body
      (c) Rejection wick: wick opposing the trend >= 40% of candle range
          (shows price rejected the level and is moving with trend)
    Softened from 0.6 body_ratio → 0.55 to match Candle.is_strong threshold.
    Wick threshold softened from 50% → 40% to catch more rejection patterns.
    """
    if len(m15) < 2:
    ...
    # (a) Strong directional candle
    if trend == "bullish" and last.is_bullish and last.body_ratio >= 0.55:
        return True, f"Strong bullish candle (body_ratio={last.body_ratio:.2f})"
    if trend == "bearish" and last.is_bearish and last.body_ratio >= 0.55:
    ...
    # (c) Rejection wick (40% threshold)
    if last.range > 0:
        if trend == "bullish" and (last.lower_wick / last.range) >= 0.40:
            return True, f"Bullish rejection wick ({last.lower_wick / last.range:.0%} lower wick)"
        if trend == "bearish" and (last.upper_wick / last.range) >= 0.40:
            return True, f"Bearish rejection wick ({last.upper_wick / last.range:.0%} upper wick)"

    return False, "No confirmation: no strong candle, engulfing, or rejection wick"
...
def _build_levels(
    ...
    """
    Compute entry, stop_loss, tp1, tp2, tp_final with strict 1:2 RR minimum.
    Entry  : midpoint of POI zone (fair institutional entry)
    SL     : just beyond sweep reference (the actual invalidation point)
    TP1    : 1:2 RR
    TP2    : 1:3 RR
    TPfinal: 1:4 RR (next major liquidity pool)
    SL buffer: 0.1% forex / 0.3% crypto to avoid stop hunting right at the level.
    ...
    sl_buffer_pct = 0.003 if market_type == "crypto" else 0.001
    ...
    # Enforce minimum 1:2 RR — if risk is tiny relative to entry, skip
...
def generate_signal(
    ...
    Gates 1, 2, 3, 4, 6, 8, 10 are HARD blocks — return WAIT on failure.
    Gates 5, 9 are SOFT — logged but never block.
    Gate 7 blocks only on confirmed REVERSAL (8/10 candles against trend).
    Gate 11 has OB → FVG → swing-extreme fallback (almost never fully blocks).
    Gate 12 flexible confirmation (strong candle OR engulfing OR rejection wick).
    """
    ...
    if trend == "bullish" and zone == "premium":
        return _wait(f"G3 Zone: Price in premium ({zone}) — wait for discount/equilibrium for BUY")
    if trend == "bearish" and zone == "discount":
        return _wait(f"G3 Zone: Price in discount ({zone}) — wait for premium/equilibrium for SELL")
    ...
    # ── GATE 5: Key level (SOFT) ──────────────────────────────────────────────
    h1_highs, h1_lows = _find_swing_highs_lows(h1, window=3)
    at_key, key_reason = _at_key_level(h1, h1_highs, h1_lows, market_type)
    if not at_key:
        log.debug("[%s] G5 SOFT miss: %s (continuing)", symbol, key_reason)
    else:
        log.debug("[%s] G5 OK: %s", symbol, key_reason)

    # ── GATE 6: H1 Liquidity sweep ────────────────────────────────────────────
    swept_h1, sweep_h1_reason, sweep_ref = _liquidity_swept(h1, trend, strict=False)
    if not swept_h1:
    ...
    # ── GATE 9: M15 liquidity sweep (SOFT — falls back to H1 ref) ────────────
    swept_m15, sweep_m15_reason, sweep_m15_ref = _liquidity_swept(m15, trend, strict=False)
    if swept_m15 and sweep_m15_ref > 0:
        sweep_ref = sweep_m15_ref   # prefer the tighter M15 reference
        log.debug("[%s] G9 OK (M15): %s", symbol, sweep_m15_reason)
    else:
        log.debug("[%s] G9 SOFT miss: falling back to H1 sweep ref (%.5f)", symbol, sweep_ref)
    ...
    # ── Build levels ──────────────────────────────────────────────────────────
    entry, sl, tp1, tp2, tp_final = _build_levels(m15, trend, poi_zone, sweep_ref, market_type)
    if entry == 0:
        return _wait("Levels: Could not compute valid entry/SL/TP (RR too tight)")

    # ── Setup quality grading ─────────────────────────────────────────────────
    has_real_sweep = "soft" not in sweep_h1_reason.lower()
    has_ob  = "OB" in poi_reason
    has_fvg = "FVG" in poi_reason
    if struct_quality == "strong" and has_real_sweep and (has_ob or has_fvg):
        quality = "A"
    ...
    log.info(
        "[%s] ✅ %s signal | entry=%.5f SL=%.5f TP1=%.5f | quality=%s",
        symbol, direction, entry, sl, tp1, quality,
    )
